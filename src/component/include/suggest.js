...
function Suggest({element, data, optionsNumber}) {
    ...
    function getList(startWith) {
        ...
        for(let item of data){
            if(item.toLowerCase().startsWith(startWith.toLowerCase()) && !(list.includes(item))){
                list.push(item);
    ...
    return(
        ...
        <ul>{suggest && suggest.length !== 0? suggest.map((elem, i)=><li onClick={handelOnclickLi} key={i}>{elem}</li>):<li>pas d'options.</li>}
        </ul>
    </div>
    );
}
